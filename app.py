...
def user_input_features():
    ...
    a2 = st.sidebar.slider('Hematocrit(%)',min_value=10, max_value=70, value=30,step=1)
    ...
    a5 = st.sidebar.slider('Blood urea nitrogen(mg/dL)', min_value=5, max_value=70, value=30,step=1)
    ...
    a9 = st.sidebar.slider('Bicarbonate(mEq/L)', min_value=10, max_value=40, value=23,step=1)
    if a4 == 'Yes':
            a4 = 1
    else:
        a4 = 0
...
st.write('User input parameters below ⬇️')
st.write(outputdf)
st.write(f'Probability of in-hospital death:{p2}')
...
with placeholder5.container():
    ...
    with f1:
        ...
        st.write('The beeswarm plot is designed to display an information-dense summary of how the top features in a dataset impact the model’s output. Each instance the given explanation is represented by a single dot on each feature fow. The x position of the dot is determined by the SHAP value of that feature, and dots “pile up” along each feature row to show density. Color is used to display the original value of a feature In the plot below we can see that MV is the most important feature on average, Higher MV make patients more likely to have in-hospital deaths.')
        st.write(' ')
        st.image(image4)     
    with f2:
